...
<body>
  ...
  $registros = mysqli_query($conexion, "select codigo from secondary_p
                        where Codigo='$_REQUEST[ID]'") or
    die("Problemas en el select:" . mysqli_error($conexion));
  if ($reg = mysqli_fetch_array($registros)) {
    mysqli_query($conexion, "delete from secondary_p where Codigo='$_REQUEST[ID]'") or
      die("Problemas en el select:" . mysqli_error($conexion));
